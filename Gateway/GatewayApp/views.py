...
class ConcreteUserDeliveryView(APIView):
    def get(self, request: Request, user_id):
        ...
        # return Response(
        #     {
        #         "error": "You can only check delivery status that belongs to currently logged user"
        #     },
        #     status=400,
        # )
...
class ConcreteOrderView(APIView):
    ...
    def delete(self, request: Request, order_uuid):
        response_json, code = Requester.delete_concrete_order(order_uuid=order_uuid)
        print("json",response_json, code)
        return Response(data=response_json, status=code)
